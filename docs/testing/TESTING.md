...
| `test_refresh_legendEntries_lastBuyAndSaleReference` | `latestBuy`/`latestSale` gesetzt (12.05.2022, 198,36€ bzw. 27.02.2020, 205,25€), ein einziger Tageswert (422,40€) | Legende enthält "Letzter Kauf"/"Letzter Verkauf" mit Datum, Preis und Entwicklung (224,04€ bzw. 217,15€) relativ zum aktuellsten Schluss-Kurs |
| `test_refresh_legendEntries_referenceUsesLatestNotMaxClosingPrice` | Regressionstest Bugfix 26.09.2026: Hoch 453,50€ in der Mitte des Zeitraums, jüngster Wert 424,40€ (Allianz-Screenshot nachgebildet) | Entwicklung bei "Letzter Kauf"/"Letzter Verkauf" rechnet mit 424,40€ (226,04€ / 113,95 % bzw. 219,15€ / 106,77 %), nicht mit dem Hoch; die Min/Max-Zeile des Schluss-Kurses bleibt bei 410,90/453,50 |
| `test_refresh_legendEntries_rangeEndBeforeLatest_referenceStillLatestClosingPrice` | Start-Datum nach dem Laden auf einen Tag vor dem jüngsten Tageswert gesetzt, Fenster enthält ein höheres Hoch (500,00€) | Bezugskurs bleibt der jüngste Schluss-Kurs (424,40€), den `currentClosingPrice()` gezielt nachlädt — weder letzter Wert noch Hoch des Fensters |
...
Achsen-Rebuild, Legende-Layout, Rendering der Kauf-/Verkauf-Markerlinien via
`setReferenceLines()`) — analog zur bestehenden Konvention, dass reine
